...
import { db } from './firebaseAdmin'; // Adjust the import path if necessary
...
export default async function handler(req, res) {
  ...
  try {
    // Check Firestore for existing TMDB ID
    const docRef = db.collection('api').doc('tmdb');
    const doc = await docRef.get();

    if (doc.exists && doc.data()[tmdbid]) {
      // If the TMDB ID exists in Firestore
      const netflixId = doc.data()[tmdbid];
    ...
    // If TMDB ID is not found in Firestore, make the API request
    ...
    if (!data.result || !data.result.streamingInfo) {
      // Save "none" to Firestore if no streaming info is found
      await docRef.set({ [tmdbid]: 'none' }, { merge: true });
    ...
    await docRef.set({ [tmdbid]: netflixId }, { merge: true });
